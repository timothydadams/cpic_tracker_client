...
export const EditRolesModal = ({ user, availableRoles }) => {
  const { id, email } = user;

  let [isOpen, setIsOpen] = useState(false);
  const [selectedRoles, setSelectedRoles] = useState([]);

  const { data: existingRoles, refetch } = useGetUserRolesQuery(id, {
    skip: !id,
  });
    ...
    availableRoles && (
      <>
        ...
        <Dialog open={isOpen} onClose={setIsOpen}>
          <DialogTitle>Details ({email})</DialogTitle>
          <DialogDescription></DialogDescription>
          <DialogBody>
            <Fieldset>
              <Legend>Modify Roles the user belongs to.</Legend>
              <Text>
                checked selections indicate existing role memberships.
              </Text>
