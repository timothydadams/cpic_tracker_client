...
**Routing:** React Router v6 with nested routes in `src/Routes.js`. All route components are lazy-loaded via `React.lazy()` with a shared `PageLoader` Suspense fallback. Since components use named exports, the lazy imports use `.then(m => ({ default: m.NamedExport }))`. Routes are wrapped in `PersistAuth` for session persistence. Protected routes use `ProtectRoute` (role-gated) and `AnonymousOnly` (login/register pages — redirects authenticated users to their previous page via `sessionStorage.returnTo` or `location.state.from`). The `/metrics` route uses nested child routes (`/metrics/overview`, `/metrics/focus-areas`, `/metrics/timelines`, `/metrics/implementers`) with a shared layout shell (`MetricsPage`) rendering `<Outlet />`.
...
**Auth state hydration:** The JWT access token contains only minimal claims (`id`, `roles`, `profile_pic`) because the backend's Prisma PII-stripping extension removes name fields during the unauthenticated sign-in lookup. After `setCredentials` stores the decoded JWT in `state.auth.user`, the `useHydrateUser` hook (`src/Hooks/useHydrateUser.js`) — called in `Layout.js` — fetches the full user profile via `GET /users/:id` (authenticated, so PII is included) and dispatches `setUserProfile` to merge `given_name`, `family_name`, `display_name`, `email`, `profile_pic` into `state.auth.user`. The `selectMemoizedUser` selector automatically picks up the merged fields. Auth selectors (`selectCurrentUserId`, `selectCurrentEmail`, `selectCurrentRoles`) use optional chaining to handle `null` user state. **Important:** `setCredentials` merges JWT claims into the existing user object (via `Object.assign`) rather than replacing it, so token refreshes (from `PersistAuth` or `baseQueryWithReauth`) preserve the hydrated profile fields.

**Post-login redirect:** `Layout.js` tracks the current path in `sessionStorage.returnTo` (skipping `/login` and `/register` paths). `AnonymousOnly` reads this value in a `useEffect` (not during render — avoids race conditions with React re-renders) and redirects authenticated users to the stored path. The `LoginForm` only dispatches `setCredentials` — all redirect logic is owned by `AnonymousOnly`. Login errors (including 429 rate limits) are displayed via `enqueueSnackbar` (notistack).
