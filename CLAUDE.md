...
**Auth state hydration:** The JWT access token contains only minimal claims (`id`, `roles`, `profile_pic`) because the backend's Prisma PII-stripping extension removes name fields during the unauthenticated sign-in lookup. After `setCredentials` stores the decoded JWT in `state.auth.user`, the `useHydrateUser` hook (`src/Hooks/useHydrateUser.js`) — called in `Layout.js` — fetches the full user profile via `GET /users/:id` (authenticated, so PII is included) and dispatches `setUserProfile` to merge `given_name`, `family_name`, `display_name`, `email`, `profile_pic` into `state.auth.user`. The `selectMemoizedUser` selector automatically picks up the merged fields. Auth selectors (`selectCurrentUserId`, `selectCurrentEmail`, `selectCurrentRoles`) use optional chaining to handle `null` user state.
